...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    console.log('recipientBalance(Layer1) = ', recipientBalance.toString())
    expect(recipientBalance).to.be.equal(aliceWithdrawAmount)

    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    console.log('omniBridgeBalance(Layer1) = ', omniBridgeBalance.toString())
    expect(omniBridgeBalance).to.be.equal(0)

    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
    console.log('tornadoPoolBalance(Layer2) = ', tornadoPoolBalance.toString())
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    //Alice withdraww a rest of her funds from the shielded pool
    const tornadoPoolBalance1 = await token.balanceOf(tornadoPool.address)
    ...
    const aliceWithdrawUtxo = new Utxo({
      amount: tornadoPoolBalance1.sub(aliceWithdrawAmount),
      keypair: aliceDepositUtxo.keypair,
    })
    await transaction({
      tornadoPool,
    ...
    console.log('aliceBalance(Layer1) = ', aliceBalance.toString())
